...
def process_submission(db: DatabaseClient, sub: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 3.6 Campaign Brief Compliance Verification (Hashtags, Mentions, Required Copy)
    requirements = campaign.get('requirements') or {}
    post_text = f"{result.title or ''} {result.description or ''}".strip()

    # 1. Required Hashtags
    req_hashtags = requirements.get('hashtags') or []
    if isinstance(req_hashtags, list) and req_hashtags:
        clean_text_lower = post_text.lower()
        missing_tags = []
        for tag in req_hashtags:
            if tag and tag.strip():
                tag_clean = tag.strip().lower()
                if tag_clean not in clean_text_lower:
                    missing_tags.append(tag.strip())
        if missing_tags:
            logger.warning(
                f"Brief compliance failed for sub {sub_id[:8]}: missing hashtag(s) {missing_tags}"
            )
            updates = {
                "last_scraped_at": now_iso,
                "status": "verified_fail",
                "failure_reason": f"Campaign brief compliance failed: Your post caption is missing required hashtag(s): {', '.join(missing_tags)}.",
            }
            db.update_submission(sub_id, updates)
            prev_pending = float(sub.get('pending_payout_amount') or 0.0)
            if prev_pending > 0 and campaign.get('id'):
            ...
            return {
                ...
                "reachable": True,
                "views": 0,
                "status": "verified_fail (missing hashtags)",
                "payout": 0,
            }

    # 2. Required Mentions
    req_mentions = requirements.get('mentions') or []
    if isinstance(req_mentions, list) and req_mentions:
        clean_text_lower = post_text.lower()
        missing_mentions = []
        for mention in req_mentions:
            if mention and mention.strip():
                m_clean = mention.strip().lower().lstrip('@')
                if f"@{m_clean}" not in clean_text_lower and m_clean not in clean_text_lower:
                    missing_mentions.append(mention.strip())
        if missing_mentions:
            logger.warning(
                f"Brief compliance failed for sub {sub_id[:8]}: missing mention(s) {missing_mentions}"
            )
            updates = {
                "last_scraped_at": now_iso,
                "status": "verified_fail",
                "failure_reason": f"Campaign brief compliance failed: Your post caption is missing required mention(s): {', '.join(missing_mentions)}.",
            }
            db.update_submission(sub_id, updates)
            prev_pending = float(sub.get('pending_payout_amount') or 0.0)
            if prev_pending > 0 and campaign.get('id'):
            ...
            return {
                ...
                "reachable": True,
                "views": 0,
                "status": "verified_fail (missing mentions)",
                "payout": 0,
            }

    # 3. Required Creative Text Copy / Keywords
    req_copy = str(requirements.get('creative_text_copy') or '').strip()
    if req_copy:
        if req_copy.lower() not in post_text.lower():
            logger.warning(
                f"Brief compliance failed for sub {sub_id[:8]}: missing required creative copy"
            )
            updates = {
                "last_scraped_at": now_iso,
                "status": "verified_fail",
                "failure_reason": f"Campaign brief compliance failed: Your post caption is missing required creative text copy: '{req_copy[:60]}...'.",
            }
            db.update_submission(sub_id, updates)
            prev_pending = float(sub.get('pending_payout_amount') or 0.0)
            if prev_pending > 0 and campaign.get('id'):
            ...
            return {
                ...
                "reachable": True,
                "views": 0,
                "status": "verified_fail (missing creative copy)",
                "payout": 0,
            }
