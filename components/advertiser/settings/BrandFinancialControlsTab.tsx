...
export default function BrandFinancialControlsTab({ data }: BrandFinancialControlsTabProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white border border-kpugi-border shadow-xs space-y-6 relative overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
            <h2 className="font-display text-xl font-bold text-kpugi-ink mt-1">Current Brand Liquidity</h2>
            ...
            <div className="flex items-baseline gap-6 mt-4">
              ...
              <div className="border-l border-kpugi-border pl-6">
                <p className="text-xs font-semibold text-slate-500">Active Escrow Locked</p>
                <p className="font-mono text-xl sm:text-2xl font-bold text-slate-700">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white border border-kpugi-border shadow-xs space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="font-display text-xl font-bold text-kpugi-ink mt-1">Low-Balance Email Alerts</h2>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100 animate-fadeIn">
                ...
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-xs font-bold font-mono transition-all border ${
                    threshold === preset
                      ...
                      : 'bg-slate-50 border-kpugi-border text-slate-700 hover:bg-slate-100'
                  }`}
                >
            ...
            <div className="relative max-w-sm mt-3">
              ...
              <input
                ...
                className="w-full pl-8 pr-4 py-3 rounded-xl border border-kpugi-border bg-slate-50 text-xs sm:text-sm font-mono text-kpugi-ink focus:bg-white focus:outline-none focus:ring-2 focus:ring-kpugi-blue/20 focus:border-kpugi-blue transition-all"
              />
            </div>
            <p className="text-[11px] text-slate-400">
              Immediate alert emails will be sent to <strong className="text-slate-700">{data.advertiser.billingEmail || data.profile.email}</strong>.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white border border-kpugi-border shadow-xs space-y-4">
        <div>
          <h2 className="font-display text-xl font-bold text-kpugi-ink mt-1">Receipts & Payment History</h2>
          <p className="text-xs text-slate-500 mt-0.5">
        ...
        <div className="pt-2">
          <Link
            href="/b/wallet"
            className="inline-flex items-center gap-2 px-5 py-3 bg-slate-50 hover:bg-slate-100 text-kpugi-ink text-xs font-bold rounded-2xl border border-kpugi-border transition-all"
          >
            <Receipt className="w-4 h-4 text-kpugi-blue" />
